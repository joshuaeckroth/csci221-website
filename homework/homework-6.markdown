...
Name your BitBucket repository **exactly** `csci221-homework6`.
...
- Grab the linked list code from `/home/jeckroth/csci221-public/homework-6`.

- When reading an MP3 file, read its filename, title, artist, year, and genre. See the ID3v1 tag specification below). Convert genre id's to strings using the `genres.h` file in `/home/jeckroth/csci221-public/homework-6/genres.h`. Include `string genres[148];` in the top of your `main.cpp` file, and execute `intialize_genres()` as the first thing in your `main()` function. For any other code (besides `main.cpp`) that requires accessing this global `genres` array, put `extern string genres[148]` at the top of that file. Look up the genre in the array only in cases where the id is `>= 0` and `< 148`. Other genre ids are considered to be in the "Unknown" genre.
...
the `find` string function). For year, check for exact matches. Include some kind of `list` command that shows all the MP3 files. The
Database class should do the search and print the results, but the
`main` function should handle the user interaction.

## Makefile

For your pleasure, I've included a successful `Makefile` below.
...
main: main.o mp3.o database.o linkedlist.o node.o
    $(CXX) $(CXXFLAGS) -o main main.o mp3.o database.o linkedlist.o node.o
